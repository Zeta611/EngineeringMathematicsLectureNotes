...
\begin{theorem}[계수(rank)--퇴화 차수(nullity) 정리\footnote{계수와 퇴화 차수에 대해서는 다음 강좌에 정의한다.}]
    벡터 공간 $V$와 $W$간의 선형 사상 $L: V \rightarrow W$에 대해 다음이 성립한다:
    \begin{equation*}
        \dim \Null (L) + \dim \Range (L) = \dim V.
    \end{equation*}
\end{theorem}

\begin{example}
    위의 예시에서 본 $L$에 대해 $\Null (L) = \left<(1, -2, 1)\right>$이고 $\Range (L) = \left<(1, 4, 7), (2, 5, 8)\right>$이어서, $\dim \Null (L) + \dim \Range (L) = 1 + 2 = 3$이 성립한다.
\end{example}
\end{document}
